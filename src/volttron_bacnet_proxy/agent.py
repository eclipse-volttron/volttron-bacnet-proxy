...
class BACnetApplication(BIPSimpleApplication, RecurringTask):
    ...
    def __init__(self, i_am_callback, send_cov_subscription_callback, forward_cov_callback,
    ...
    def _get_iocb_for_apdu(self, apdu, invoke_key):
        ...
        if isinstance(apdu, AbortPDU):
            ...
            working_iocb.set_exception(
                RuntimeError("Error during device communication: " + str(apdu)))
            return None
    ...
    def confirmation(self, apdu):
        ...
        if isinstance(working_iocb.ioRequest, ReadPropertyRequest) and isinstance(
            ...
            if apdu.propertyIdentifier == 'covIncrement':
                _log.debug("received read covIncrement property response from {}".format(
                    apdu.pduSource))
                subscription = None
                subscription_id = -1
                ...
                if subscription:
                    if value:
                        _log.info(
                            f"covIncrement is {value} for point {subscription.point_name} on device {subscription.device_path}"
                        )
                        self.send_cov_subscription_callback(
                            apdu.pduSource, subscription.subscriberProcessIdentifier,
                            subscription.monitoredObjectIdentifier, subscription.lifetime,
                            subscription.point_name)
                    else:
                        _log.warning(
                            "point {} on device {} does not have a valid covIncrement property")
                        self.bacnet_application.sub_cov_contexts.pop(subscription_id)
                else:
                    _log.error(
                        'Received read covIncrement response, but no subscription context exists for {} on {}'
                        .format(subscription.device_path, subscription.point_name))
            else:
                working_iocb.set(value)
            return

        elif isinstance(working_iocb.ioRequest, WritePropertyRequest) and isinstance(
                apdu, SimpleAckPDU):
            working_iocb.set(apdu)
            return

        # Simple record-keeping for subscription request responses
        elif isinstance(working_iocb.ioRequest, SubscribeCOVRequest) and isinstance(
                apdu, SimpleAckPDU):
            ...
            return
                        ...
                        SubscribeCOVRequest) and not isinstance(apdu, SimpleAckPDU):
            ...
            for result in apdu.listOfReadAccessResults:
                ...
                for element in result.listOfResults:
                    ...
                    if read_result.propertyAccessError is not None:
                        ...
                        _log.error(
                            msg.format(error_obj.errorClass, error_obj.errorCode,
                                       object_identifier))
                        ...
                        if issubclass(datatype, Array) and property_array_index is not None:
                            ...
                            value = self._get_value_from_property_value(
                                property_value, datatype, working_iocb)

                        result_dict[object_identifier[0], object_identifier[1],
                                    property_identifier, property_array_index] = value

            working_iocb.set(result_dict)

        else:
            _log.error(
                "For invoke key {key} Unsupported Request Response pair Request: {request} Response: {response}"
    ...
    def indication(self, apdu):
        if isinstance(apdu, IAmRequest):
            ...
            self.i_am_callback(str(apdu.pduSource), device_instance, apdu.maxAPDULengthAccepted,
            ...
            if point_name and device_path:
                ...
                _log.debug("Device {} does not have a subscription context.".format(
...
class BACnetProxyAgent(Agent):
    ...
    def __init__(self,
        ...
        self.setup_device(async_call, device_address, max_apdu_len, seg_supported, obj_id,
                          obj_name, ven_id, request_check_interval)
    ...
    def setup_device(self,
        ...
        def i_am_callback(address, device_id, max_apdu_len, seg_supported, vendor_id):
            async_call.send(None, self.i_am, address, device_id, max_apdu_len, seg_supported,
                            vendor_id)

        def send_cov_subscription_callback(device_address, subscriber_process_identifier,
            ...
            async_call.send(None, self.send_cov_subscription, device_address,
                            subscriber_process_identifier, monitored_object_identifier, lifetime,
                            point_name)
        ...
        self.bacnet_application = BACnetApplication(i_am_callback, send_cov_subscription_callback,
                                                    forward_cov_callback, request_check_interval,
                                                    this_device, address)
    ...
    def i_am(self, address, device_id, max_apdu_len, seg_supported, vendor_id):
        ...
        """
        _log.debug(
            "IAm received: Address: {} Device ID: {} Max APDU: {} Segmentation: {} Vendor: {}".
            format(address, device_id, max_apdu_len, seg_supported, vendor_id))

        header = {headers.TIMESTAMP: format_timestamp(datetime.datetime.utcnow())}
        value = {
            "address": address,
            "device_id": device_id,
    ...
    def forward_cov(self, device_path, point_name, result_dict):
        ...
        """
        self.vip.rpc.call(PLATFORM_DRIVER, 'forward_bacnet_cov_value', device_path, point_name,
                          result_dict)
    ...
    def read_using_single_request(self, target_address, point_map):
        ...
        for point, properties in point_map.items():
            if len(properties) == 3:
                ...
                _log.error("skipping {} in request to {}: incorrect number of parameters".format(
                    point, target_address))
                continue

            try:
                results[point] = self.read_property(target_address, object_type, instance_number,
    ...
    def _get_access_spec(self, obj_data, properties):
        ...
        for prop, prop_index in properties:
            ...
            count += 1
        return (ReadAccessSpecification(objectIdentifier=(obj_type, obj_inst),
    ...
    def read_properties(self,
        ...
        # reverse_point_map
        (object_property_map,
         reverse_point_map) = self._get_object_properties(point_map, target_address)
        ...
        while not finished:
            ...
            if read_access_spec_list:
                _log.debug("Requesting {count} properties from {target}".format(
    ...
    def create_cov_subscription(self,
        ...
        if not isinstance(address, str):
            raise RuntimeError(
                "COV subscriptions require the address of the target device as a string")
        ...
        if subscription:
            self.send_cov_subscription(subscription.device_address,
                                       ...
                                       subscription.lifetime, subscription.point_name)
        else:
            subscription = SubscriptionContext(device_path, Address(address), point_name,
                                               object_type, instance_number,
                                               self.bacnet_application.cov_sub_process_ID,
                                               lifetime)
            # check whether the device has a usable covIncrement
            try:
                _log.debug("establishing cov subscription for point {} on device {}".format(
                    point_name, device_path))
                self.bacnet_application.sub_cov_contexts[
                    self.bacnet_application.cov_sub_process_ID] = subscription
                self.bacnet_application.cov_sub_process_ID += 1
                _log.debug("sending read property request for point {} on device {}".format(
                    point_name, device_path))
                self.read_property(address, object_type, instance_number, 'covIncrement')
            except Exception as error:
                _log.warning(
                    "the covIncrement for {} on {} could not be read, no cov subscription was established"
