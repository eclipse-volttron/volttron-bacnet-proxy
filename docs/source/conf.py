...
project = 'VOLTTRON BACnet Proxy Agent'
...
def setup(app):
    ...
    app.connect('builder-inited', generate_external_docs)

    # app.connect('build-finished', clean_api_rst)
    app.connect('build-finished', clean_external_docs_rst)


def generate_external_docs(app):
    os.makedirs(external_docs_root)
    project_name = agent_name= 'volttron-lib-bacnet-driver'
...
def clean_external_docs_rst(app, exception):
    """
